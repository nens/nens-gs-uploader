...
def match_sld_fields(sld_field_name, shape_object):
    ...
    for shape_field_name in shape_object.get_all_field_names():
        matches[shape_field_name] = percentage_match(shape_field_name, 
                                                   sld_field_name)
...
def replace_sld_field_based_on_shape(shape_object, sld_object, 
                                                             sld_field_name):
    """ Replaces sld field with most matching fieldnames in shapefile. """
    
    print(""" apparently not a perfect match of sld in shape,  
          searching for a high percent match """)
        
    shape_field_name, match_value = match_sld_fields(sld_field_name, 
                                                         shape_object)
    if match_value < 0.8:
