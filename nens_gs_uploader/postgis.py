...
def copy2pg_database(database, in_layer, layer_name):
    try:
        out_layer = database.datasource.CopyLayer(
            in_layer,
            layer_name,
            ["OVERWRITE=YES", "SCHEMA=public", "SPATIAL_INDEX=GIST", "FID=ogc_fid"],
        )
        ...
        database.datasource.ExecuteSQL(
            'CREATE INDEX idx ON "{}"' "USING GIST (wkb_geometry);".format(layer_name)
        )
