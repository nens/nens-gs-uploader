...
class Catalog:
    ...
    def about(self):
        ...
        raise FailedRequestError(
            "Unable to determine version: {}".format(r.text or r.status_code)
        )
    ...
    def get_xml(self, rest_url):
        ...
        try:
            ...
            msg = "GeoServer gave non-XML response for [GET {}]: {}".format(
                rest_url, text
            )
    ...
    def create_coveragestore_external_geotiff(
        ...
        self._create_coveragestore(
            name, data, workspace=workspace, overwrite=overwrite, external=True
        )

    def _create_coveragestore(
        self, name, data, workspace=None, overwrite=False, external=False
    ):
        if not overwrite:
            try:
    ...
    def mosaic_coverage_schema(self, coverage, store):
        ...
        cs_url = urljoin(
            ...
            "workspaces/{}/coveragestores/{}/coverages/{}/index.json".format(
                store.workspace.name, store.name, coverage
            ),
        )
    ...
    def create_workspace(self, name):
        xml = "<workspace><name>{name}</name></workspace>".format(name=name)
        headers = {"Content-Type": "application/xml"}
        workspace_url = urljoin(self.service_url, "workspaces/")
        print(workspace_url, xml, headers)
        print(self.session)
