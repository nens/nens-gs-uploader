...
def get_lizard_raster(raster_uuid):
    """return json of raster based on raster uuid"""
    r = requests.get(
        url="{}rasters/{}".format(LIZARD_URL, raster_uuid), headers=get_headers()
    )
    r.raise_for_status()
    return r.json()
    

def create_raster_task(raster, target_srs, resolution=None, bounds=None, time=None, width=None, height=None, lizard_url=LIZARD_URL):
    ...
    if lizard_url == LIZARD_URL:
        ...
        v = list(bounds.values())
        geometry = {"bbox": "{},{},{},{}".format(v[0], v[2],
                                                 v[1], v[3])}
...
def download_lizard_raster(
    raster_uuid, target_srs, resolution=None, bounds=None, width=None, height=None, time=None, pathname=None,
    ...
    raster = get_lizard_raster(raster_uuid)

    task = create_raster_task(raster, target_srs, resolution, bounds, time,
                              width, height,
                              lizard_url = LIZARD_URL_V4)
    
    task_uuid = task["task_id"]

    log.debug("Start waiting for task {} to finish".format(task_uuid))
    while (get_task_status(task_uuid) == "PENDING" or 
           get_task_status(task_uuid) == "STARTED"):
        sleep(5)
        print("Still waiting for task {}".format(task_uuid))
        log.debug("Still waiting for task {}".format(task_uuid))
...
if __name__ == '__main__':
    raster_uuid = 'f2052db8-598f-4f1c-b07a-7a2236fe8dbf'
    target_srs = 4326
    resolution = 0.5    
    pathname = "C:/Users/chris.kerklaan/Documents/temp/waterdepth.tif"
    raster = get_lizard_raster(raster_uuid)
    
    download_lizard_raster(raster_uuid, target_srs, resolution,
                           bounds=None, time=None, pathname=pathname)