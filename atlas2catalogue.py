...
# relevant paths
SCRIPTS  = "C:/Users/chris.kerklaan/Documents/base_modules"
...
from nens_gs_uploader.postgis import PG_DATABASE
...
from gis_base.vector import wrap_shape,  reproject
from gis_base.sld import wrap_sld
...
from klimaatatlas.localsecret import production_klimaatatlas_v1 as pg_atlas_v1
from klimaatatlas.localsecret import production_klimaatatlas as pg_atlas_v2
from klimaatatlas.localsecret import project_klimaatatlas as pg_atlas_project
from klimaatatlas.localsecret import project_lizard as pg_lizard
from klimaatatlas.api import wrap_atlas

from lizard.rasterstore import rasterstore
from lizard.wmslayers import wmslayers
from lizard.geoblocks import clip_waterschappen, clip_gemeentes ,clip_provincies
from lizard.geoblocks import uuid_store
...
def vector_in_data_directories(name, subdir):  
    ...
    #check in subdirs
    locations = [pg_atlas_v1['folder'], 
                pg_atlas_v2['folder'],
                pg_atlas_project['folder'],
                pg_lizard['folder']] 
                
                
    paths_subdir = [glob(location + "/{}/*.shp".format(subdir)) for location in locations]
    paths_general = [glob(location + "/**/*.shp".format(subdir)) for location in locations]
    paths_cover = [glob(location + "/**/*.tif".format(subdir)) for location in locations]
...
def get_uuid_by_organisation(rasters, organisation = 'Nelen & Schuurmans'):
    ...
    while pages:
        print('Searching page {}, {} pages for organisation {}'.format(str(page),
              str(page_size),organisation))
...
def extract_vector_data(vectors, temp_dir, organisation):
    ...
    for vector in tqdm(vectors):       
        try:     
            ...
            vector['extract_error'] =  "missing sld body layer not in geoserver , {}".format(e) 
            ...
            vector['extract_error'] = "Found coverage store instead of vector{}".format(e)
            vector['temp_path'] = None
            extract_data_failures.append(vector)
...
def upload_ready(temp_dir, upload_dir, clip_geom, organisation, bo_nummer, epsg):
    ...
    for meta_path in tqdm(glob(temp_dir +"/*.json")):
        ...
        vector_name = get_subject_from_name(os.path.basename(vector_path).split(".")[0], organisation)
        ...
        meta_data['slug_new'] = "{}:{}_{}_{}".format(dataset, bo_nummer, organisation, vector_name_new)
...
def upload_data(upload_dir, bo_nummer, organisation, eigen_naam, project_nummer):
    create_nens_gs_uploader_instellingen(upload_dir, 
    ...
    for vector_path in tqdm(glob(upload_dir +"/*.gpkg")):
        ...
        config.set(os.path.basename(vector_path),'abstract',strip_information( meta_data['information']))
    ...
    os.system('python C:/Users/chris.kerklaan/tools/nens_gs_uploader.py {}'.format(ini_file_title))
...
def create_wmslayers(upload_dir, organisation, use_nens=True):
    ...
    for meta_path in glob(upload_dir +"/*.json"):            
            ...
            if error:
                ...
                url = 'https://maps1.klimaatatlas.net/geoserver/{}_klimaatatlas/wms'.format(organisation)
               
            # set dataset
            dataset = "{}_klimaatatlas".format(organisation)
            
            # download link
            download_url = "{}?&request=GetFeature&typeName={}&OutputFormat=shape-zip".format(url.replace('wms', 'wfs'),slug)
...
def atlas2catalogue_rasterstores(rasters, clip_list,bo_nummer, organisation, use_nens=False):
...
#            else:
        store_configuration =  store.get_store(search_terms , raster['slug'],
                                                   method="uuid", uuid= raster['uuid'])
        
        # copy rasterstore
        configuration_new = store_configuration[0]
        
        # add clip top copied rasterstore
        geoblock = uuid_store(configuration_new['uuid'])
        ...
        # observation type 
        if not 'observation_type' in configuration_new:
            print('No observation type, using waterdepth')
            code = 'Waterdepth'
            #raise ObservationTypeNotInStore('Observation type not in store')
        elif  not 'code' in configuration_new['observation_type']:
            print('No observation type, using waterdepth')
            code = 'Waterdepth'
        ...
        # add datasets        
        configuration_new["datasets"] = ["{}_klimaatatlas".format(organisation)]
        
        # add slug for search
        configuration_new["slug"] = slug
        
        # create stores
        store.create(configuration_new, overwrite=True)
#        
#        except TypeError:
#            print("raster not found in rasterstore")
...
if __name__ == "__main__":
     #input arugments
    atlas_name = "westland"
    area_path = "C:/Users/chris.kerklaan/Documents/Projecten/westland/westland.shp"
    ...
    #geoserver, geoserver_data = get_geoserver_data('PRODUCTIE_KLIMAATATLAS')
    clip_geom = vector_to_geom(area_path, epsg=3857)
    
    # extract vector data from their respective sources
    extract_succes, extract_failure = extract_vector_data(vectors, temp_dir, organisation)
    
    # make them upload ready 
    ready_succes, ready_failure = upload_ready(temp_dir, upload_dir, clip_geom, organisation, bo_nummer, epsg=3857)
    
    # upload them to the geoserver
    upload_data(upload_dir, bo_nummer, organisation, eigen_naam, project_nummer)
    
    # create wms layers
    wmslayer_succes, wmslayer_failures = create_wmslayers(upload_dir, organisation, use_nens=False)
    ...
    rasters[6]['uuid'] = 'f50e8ad6-66cf-4247-9188-7cde3c0e976f'
    rasters[7]['uuid'] = '5aad9db6-7b71-49aa-9759-7dad26802c3c'
    rasters[8]['uuid'] = '1d65a4e1-ac2f-4e66-9e52-1d130d870a34'
    rasters[9]['uuid'] = 'f33a04b1-44c5-4a70-a46b-d174e32accc3'
    
    raster_succes, raster_failures = atlas2catalogue_rasterstores(rasters,
                                                                  raster_clip_id, bo_nummer, organisation, use_nens=True)
